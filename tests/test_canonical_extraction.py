...
from linkcleaner.link_resolver import CANONICAL_LINK_RE, OG_URL_RE
...
def test_meta_refresh_extracts_url():
    ...
    match = link_resolver.META_REFRESH_RE.search(html)
    assert match is not None
    assert match.group(1) == "https://www.facebook.com/reel/123/"
...
def test_json_escaped_url_extracts_and_unescapes():
    ...
    match = link_resolver.JSON_ESCAPED_URL_RE.search(html)
    assert match is not None
